...
class Solution:
    def minEatingSpeed(self, piles: list[int], H: int) -> int:
        # piles 数组的最大值
        max_pile = self.getMax(piles)
        for speed in range(1, max_pile):
...
class Solution:
    def shipWithinDays(self, weights: list[int], D: int) -> int:
        # 载重可能的最小值
        left = self.getMax(weights)
        # 载重可能的最大值 + 1
        right = self.getSum(weights) + 1
        while left < right:
            mid = left + (right - left) // 2
    ...
    def canFinish(self, w: list[int], D: int, cap: int) -> bool:
        ...
        for _ in range(D):
            max_cap = cap
            while (max_cap := max_cap - w[i]) >= 0:
                i += 1
                if i == len(w):
                    return True
        return False

    def getSum(self, weights: list[int]) -> int:
        return sum(weights)
```python
