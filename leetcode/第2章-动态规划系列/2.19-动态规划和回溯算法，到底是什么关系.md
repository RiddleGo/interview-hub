...
```python
def backtrack(nums: list[int], i: int) -> None:
    if i == len(nums):
        if rest == 0:
            result += 1
        return
    for op in (+1, -1):
        # 选择 op * nums[i]，穷举 nums[i + 1]
        backtrack(nums, i + 1)
        # 撤销选择
```python
...
class Solution:
    def findTargetSumWays(self, nums: list[int], target: int) -> int:
        if not nums:
            return 0
        self.result = 0
        self._backtrack(nums, 0, target)
        return self.result

    def _backtrack(self, nums: list[int], i: int, rest: int) -> None:
        if i == len(nums):
            if rest == 0:
                self.result += 1
            return
        rest += nums[i]
        self._backtrack(nums, i + 1, rest)
        rest -= nums[i]
        rest -= nums[i]
        self._backtrack(nums, i + 1, rest)
        rest += nums[i]
...
```python
def backtrack(nums: list[int], i: int, total: int, target: int) -> None:
    if i == len(nums):
        if total == target:
            result += 1
        return
...
```python
def backtrack(nums: list[int], i: int, rest: int) -> None:
    if i == len(nums):
        return
...
```python
def backtrack(i: int, rest: int) -> None:
...
class Solution:
    def findTargetSumWays(self, nums: list[int], target: int) -> int:
        ...
        def dp(i: int, rest: int) -> int:
            if i == len(nums):
                return 1 if rest == 0 else 0
            key = (i, rest)
            if key in memo:
                return memo[key]
            result = dp(i + 1, rest - nums[i]) + dp(i + 1, rest + nums[i])
            memo[key] = result
            return result
...
```python
def subsets(nums: list[int], target_sum: int) -> int:
    """计算 nums 中有几个子集的和为 target_sum"""
    ...
```python
然后，可以这样调用这个函数：
