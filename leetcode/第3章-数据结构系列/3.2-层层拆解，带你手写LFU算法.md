...
LRU 算法的核心数据结构是使用哈希链表 `OrderedDict`，首先借助链表的有序性使得链表元素维持插入顺序，同时借助哈希映射的快速访问能力使得我们可以在 O(1) 时间访问链表的任意元素。
...
1、使用一个 `dict` 存储 `key` 到 `val` 的映射，就可以快速计算 `get(key)`。

```python
key_to_val = {}
```python
2、使用一个 `dict` 存储 `key` 到 `freq` 的映射，就可以快速操作 `key` 对应的 `freq`。
