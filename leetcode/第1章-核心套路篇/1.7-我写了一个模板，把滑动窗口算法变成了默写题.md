...
本文代码为 Python 实现，不会用到什么编程方面的奇技淫巧，但是还是简单介绍一下一些用到的数据结构，以免有的读者因为语言的细节问题阻碍对算法思想的理解：

`unordered_map` 就是哈希表（字典），它的一个方法 count(key) 相当于 containsKey(key) 可以判断键 key 是否存在。

可以使用方括号访问键对应的值 `map[key]`。需要注意的是，普通 `dict` 在 key 不存在时会抛 `KeyError`；若需要「不存在则当作 0」的语义，可用 `collections.defaultdict(int)`，或 `map[key] = map.get(key, 0) + 1`。

所以代码中多次出现的 `map[key] += 1` 在 `defaultdict(int)` 下与「不存在则按 0 计数」的写法等价。
...
while right < len(s):
    ...
    right += 1
    # 如果符合要求，移动 left 缩小窗口
    while window_valid:  # window 符合要求
        # 如果这个窗口的子串更短，则更新 res
        res = min_len(res, window)
