...
def sort_cakes(cakes: List[int], n: int) -> None:
    ...
```python
...
class Solution:
    def pancakeSort(self, cakes: List[int]) -> List[int]:
        self.res: List[int] = []
        self._sort(cakes, len(cakes))
        return self.res

    def _sort(self, cakes: List[int], n: int) -> None:
        if n == 1:
            return
        ...
        self._reverse(cakes, 0, max_cake_index)
        self.res.append(max_cake_index + 1)
        self._reverse(cakes, 0, n - 1)
        self.res.append(n)

        self._sort(cakes, n - 1)

    def _reverse(self, arr: List[int], i: int, j: int) -> None:
        while i < j:
