...
# n 现在就等于 458
```python
这个还是很简单的吧，老套路了。但是即便这么简单，依然有坑：**`(ord(c) - ord('0'))` 的运算顺序不能搞错，否则可能造成整型溢出**。

因为变量 `c` 是一个字符，如果不加括号就会先加后减，想象一下 `s` 如果接近 极大值，就会溢出。所以用括号保证先减后加才行。
...
def calculate(s: str) -> int:
    ...
    return sum(stk)
```python
我估计就是中间带 `if sign` 分支的部分有点不好理解吧，`i` 就是从左到右扫描，`sign` 和 `num` 跟在它身后。当 `s[i]` 遇到一个运算符时，情况是这样的：

![](../pictures/calculator/1.jpg)

所以说，此时要根据 `sign` 的不同选择 `num` 的正负号，存入栈中，然后更新 `sign` 并清零 `num` 记录下一对儿符号和数字的组合。

另外注意，不只是遇到新的符号会触发入栈，当 `i` 走到了算式的尽头（`i == len(s) - 1`），也应该将前面的数字入栈，方便后续计算最终结果。
...
比如上述例子就可以分解为`+2`，`-3`，`*4`，`+5`几对儿，我们刚才不是没有处理乘除号吗，很简单，**其他部分都不用变**，在 `if sign` 部分加上对应的 case 就行了：
...
为了规避编程语言的繁琐细节，下面用 Python 实现完整计算器（含括号），核心仍是栈 + 递归：

```python
from typing import List

def calculate(s: str) -> int:
    def helper(chars: List[str]) -> int:
        stk: list[int] = []
        sign = '+'
        num = 0
...
```python
这段代码从左到右遍历字符串（通过不断从左边 `pop` 字符），本质与前面的栈解法相同。
...
现在的问题是，递归的开始条件和结束条件是什么？**遇到`(`开始递归，遇到`)`结束递归**（见上方 `helper` 中 `c == '('` 与 `c == ')'` 分支）。
